...
const ProductCard = ({ product }) => {
  ...
  const handleAddToCart = () => {
    ...
    if (typeof product.price === "string") {
      ...
      console.warn("Invalid price format, using 0 as fallback:", product.price);
      parsedPrice = 0;
    }
    if (isNaN(parsedPrice)) {
      console.error("Failed to parse price:", product.price);
      parsedPrice = 0;
    }
    const productToAdd = {
      ...product,
  ...
  const imageSrc = `${imageUrl}${product.image}.${product.image?.includes("png")? "png" : "jpg"}`
